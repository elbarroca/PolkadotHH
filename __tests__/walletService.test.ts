...
describe('WalletService', () => {
  ...
  describe('MetaMask Connection', () => {
    ...
    it('should throw error when MetaMask is not installed', async () => {
      ...
      await expect(walletService.connectMetaMask())
        .rejects
        .toThrow(new WalletError(
  ...
  describe('WalletConnect Integration', () => {
    ...
    it('should handle WalletConnect connection cancellation', async () => {
      const mockWCProvider = new WalletConnectProvider({});
      mockWCProvider.enable.mockRejectedValueOnce(new Error('User closed modal'));

      await expect(walletService.connectWalletConnect())
        .rejects
        .toThrow(new WalletError(
  ...
  describe('Wallet Disconnection', () => {
    ...
    it('should handle disconnection errors gracefully', async () => {
      const mockWCProvider = new WalletConnectProvider({});
      mockWCProvider.disconnect.mockRejectedValueOnce(new Error('Disconnect Error'));
      (walletService as any).wcProvider = mockWCProvider;

      await expect(walletService.disconnectWallet())
        .rejects
        .toThrow(new WalletError(
