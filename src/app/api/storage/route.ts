...
export async function DELETE(request: Request) {
  ...
  if (!walletAddress || !metadataId || !type) {
    return NextResponse.json({ error: "Wallet address, metadata ID, and type are required" }, { status: 400 });
  }
...
async function deleteFolderRecursively(walletAddress: string, folderId: string) {
  ...
  const childFolders = await userDocRef.collection("folders").where("parentFolder", "==", folderId).get();
